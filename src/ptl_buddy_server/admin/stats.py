...
def _stats_query(db, q='', category_ids=None, has_cat_filter=False,
    ...
    qty_col     = func.sum(Transaction.quantity).label('total_qty')
    revenue_col = func.sum(Transaction.quantity * Transaction.price_per_item).label('total_revenue')
    query = (db.query(Item, qty_col, revenue_col)
               .join(Transaction, Transaction.element_id == Item.id)
               .outerjoin(Category, Item.category_id == Category.id)
               .filter(Transaction.canceled_date != None)
               .filter(Item.id < 999)
               .group_by(Item.id))
    if q:
...
def _stats_chart_data(db, q='', category_ids=None, has_cat_filter=False, date_from='', date_to=''):
    date_label = func.strftime('%Y-%m-%d', Transaction.date).label('day')
    qty_col    = func.sum(Transaction.quantity).label('total_qty')
    query = (db.query(date_label, qty_col)
               .join(Item, Transaction.element_id == Item.id)
               .filter(Transaction.canceled_date != None)
               .filter(Item.id < 999)
